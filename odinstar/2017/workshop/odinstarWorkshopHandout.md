# OdinSTAR 2017 Workshop

<!-- footer: @EvilTester | http://compendiumdev.co.uk/page/atOdinStar2017 -->
<!-- page_number: true -->
...
- 08:30 - 08:40  General Overview (TELL)
    * Intro/Welcome
- 08:40 - 09:00 Me demo of Dev Tools and stuff (SHOW)
    * Demo/Explanations
- 09:00 - 09:30 Follow along Demo and Tutorial (DO)
    * Demo/Worked Examples

remainder

- 09:30 - 12:30 Guided Instructions (TELL / SHOW / DO)
   - split into 30 min chunks for different games and challenges & Debriefs
...
[http://compendiumdev.co.uk/page/atOdinStar2017](http://compendiumdev.co.uk/page/atOdinStar2017)
...
The Ultimate Jet Pack Hover Experience. You are a jet pack test pilot that must hover in certain positions to gain points. But no-one told you what those positions are, so boost to adjust your height and when you are in the correct position you will score points. Beware you have limited boost but your boost will reset at the same time as the correct position. Can you score more than anyone else?

- Thrill to the updating text dashboard
- Gasp as you run out of boost seconds before reset
- Groan as you realise you wasted your boost
...
"Number Hover Text Game" from [compendiumdev.co.uk/games/buggygames](http://compendiumdev.co.uk/games/buggygames)

---

# Number Hover Text Hints & Suggestions

### Gameplay Hints

* Watch the Points and when they change you have found the magic number
* Tap The Up Key To Hover The Points
* When the Points Change, hold down up to boost to the top
* Don't hold down up when count is at 99 otherwise you will waste your boost
...
* Look at the code
* Have you amended the DOM with Inspect Browser Dev Tools?
* What does debug mode do?
* There are so many ways to cheat at this game - can you get a high score without amending the `points` score variable?

Play/Hack/Test "Number Hover Text Game" from [compendiumdev.co.uk/games/buggygames](http://compendiumdev.co.uk/games/buggygames)

---

# Number Hover Text - Debrief
...
# Number Hover Text Game - Some _Obvious_ Secrets

- Debug Mode is accessible through Dom Manipulation

---

# Number Hover Text Game - Look at the source

---

#  Number Hover Text - In Memory Hack Targets

* `theCount`
    - the current count down value
* `magicNumber`
    - the number to hover around (you could set it to a known value)
* `boost`
    - to increase your boost
* `boostCost`
    - you could have infinite boost

All of these can  be amended at the console.
...
# Just Enough JavaScript To Be Dangerous - JavaScript Objects

~~~~~~~~
var aNewObject = {};
aNewObject.property = "bob";
~~~~~~~~

 **Just Enough JavaScript To Be Dangerous**
- creates a new object called `aNewObject`
- adds a field called `property` to the object and sets it to `"bob"`
- any code that accesses `aNewObject.property` would find the value `"bob"`

---

#  Number Hover Text Bot Suggestions & Challenges

* Create a Bot that supplies more boost when you get low
* Create a Bot that prevents you moving too far away from the Magic Number
* Create a Bot that prevents you hitting zero
* Create a Bot that displays the Magic Number on screen
* Create a Bot that plays the game without needing any other bots
* Create a Bot that plays the game without 'cheating'

_Combine Bots and/or Cheats for Mega Points_

---

## DOM Manipulation Explored

On the Instructions Page, if we inspect element and look at the `div` with `id` of `defaults` then we can amend these values to configure the game.

* set `debugMode` to `1` then the game uses the values in this section as defaults
* e.g.
    * give yourself more boost to make the game easier
    * increase 'points change after' to give you more time to hover over the magic number

---

## In Memory Variable Manipulation

Using the console it is possible to amend many of the variables as the game plays e.g.

* `theCount=magicNumber+2`
   * would continue to reset `theCount` to 'just above' the magic number so when the count down decrements automatically you will score points.
...
~~~~~~~~
clearInterval(myMagicNumberRangeBot);
~~~~~~~~
...
~~~~~~~~
clearInterval(myNeverZeroBot);
~~~~~~~~


---

# Show Magic Number Bot

The JavaScript code has some useful functions that we could re-use for our own purposes. e.g. `changeSpanText` which the game uses to show stats.

I could setup a bot to show the Magic Number `magicNumber` instead of the Boost.
...
~~~~~~~~
clearInterval(myMagicNumberBot);
~~~~~~~~

---

# Gameplay Bot that 'cheats'

* Create a Bot that plays the game without any other bots
* It 'cheats' because it 'knows' what the magicNumber is
...
~~~~~~~~
clearInterval(myFirstGameplayBot);
~~~~~~~~
...
It is another single page application. But it is complicated by the fact that it uses a JavaScript framework.

Unfortunately that means we really need to learn the technology that it is built on in order to test it properly.

* [BackboneJS.org](http://backbonejs.org/)

But even if we don't know the framework we can still interact with the application to help us with our testing.

* [TodoMVC.com](http://todomvc.com/)
* [todomvc.com/examples/backbone](http://todomvc.com/examples/backbone/)
...
# Explore The Code - Hard to understand if don't know backbone

- `examples/backbone` the main code folder
- `/js`
    * `collections` - the instantiated 'models'
    * `models` - probably the domain model 
    * `routers` - how it is wired together
    * `views` - how things are displayed
...
# Explore the Event Listeners from Developer View

Easier to understand because contextual.

- Jump to code in app-view.js
- can see roughly how to create
- can see how to loop around with `.each`

---

# Explore From the Console

~~~~~~~~
> app
< Object {todos: child, TodoRouter: child, TodoFilter: ""}
~~~~~~~~

~~~~~~~~
> app.todos
< child {length: 2, models: Array[2], _byId: Object
         , _listenId: "l2", _events: Object}
~~~~~~~~
...
* create todos (see walkthrough on next slide)
* select todos
* delete todos
* amend todos
...
Using information from 'find' on `examples/backbone' for "create" in `app-view.js`.

~~~~~~~~
app.todos.create(this.newAttributes());
~~~~~~~~

* Examine the `newAttributes` function and learn:

~~~~~~~~
app.todos.create({title:"auto todo", 
				order: app.todos.nextOrder(),
				completed: false})
~~~~~~~~
...
The clue for amendment is in the `todo.js`. The `toggle` function uses a `save` function, which takes an object with the properties we want to amend:

~~~~~~~~
app.todos.models[0].save({title:"bob"})
~~~~~~~~

Amend existing details:

~~~~~~~~
app.todos.models[0].save(
{title: app.todos.models[0].get("title").concat("X")}
)
~~~~~~~~

---

# Exercises  - Create, Toggle, Delete, Amend

* use a `for` loop to create 100 todos
* use a `for` loop to `toggle` all of your todos and mark them as `completed`, then use the GUI to `Clear completed`
* create another 100 , then write code to toggle half of them
* create another 100, write code to toggle every second todo
* create another 100 todos, then amend them all to have "*TODO*" after the name, e.g. "todo" would become "todo*TODO*"
* create another 100 todos, then delete them all
* create another 100, complete half, delete all completed items


---

# Answer - use a `for` loop to create 100 todos
...
# Answer - `toggle` all of your todos and mark them as `completed`

* use a `for` loop to `toggle` all of your todos and mark them as `completed`, then use the GUI to `Clear completed`

~~~~~~~~
for (x = 0; x < app.todos.models.length; x++) {
    app.todos.models[x].toggle()
}
~~~~~~~~

---

# Answer - `toggle` all of todos to mark them as `completed`
...
_Code found by using 'inspect' and 'event listeners'_

# Answer - write code to toggle half of them

* create another 100 todos, then write code to toggle half of them

~~~~~~~~
for (x = 0; x < (app.todos.models.length / 2); x++) {
    app.todos.models[x].toggle()
}
~~~~~~~~
...
# Answer - Toggle Every Second Todo

* create another 100 todos, then write code to toggle every second todo
...
~~~~~~~~
var toggleThisOne = false;
for (x = 0; x < app.todos.models.length; x++) {
    if (toggleThisOne) {
        app.todos.models[x].toggle()
    }
    toggleThisOne = !toggleThisOne;
}
~~~~~~~~

---

# Answer - Amend them all
...
~~~~~~~~
for (x = 0; x < app.todos.models.length; x++) {
  app.todos.models[x].save({
     title: app.todos.models[x].get("title").concat("*TODO*")
  })
}
~~~~~~~~
...
Need to work backwards through the array as it is changing as we delete.

---

# Answer - Delete the completed items

* create another 100 todos, complete half of them, then delete the completed items

The `completed()` function returns an array each time it is called.

~~~~~~~~
while (app.todos.completed().length > 0) {
    app.todos.completed()[0].destroy();
}
~~~~~~~~
